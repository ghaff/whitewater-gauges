...
#Gage ht. ft. variableID 45807202
        ...
        if variablecode == '45807202':
            db.gaugedata.update({"_id":agaugenum,{$set:{"height":variablevalue}})

#Discharge cfs variableID 45807197

        if variablecode == '45807197':
            db.gaugedata.update({"_id":agaugenum,{$set:{"flow":variablevalue}})
         
#save creation time so that we can throw out any stale data
    

        db.gaugedata.update({"_id":agaugenum,{$set:{"timestamp":creationtime}})
